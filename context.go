...
func CtxCreate(flags uint, dev Device) Context {
	var ctx C.CUcontext
	var params C.CUctxCreateParams
	// For CUDA v13.0, we need to pass CUctxCreateParams to use default behavior
	err := Result(C.cuCtxCreate(&ctx, &params, C.uint(flags), C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
