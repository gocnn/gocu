...
const (
	...
	CuDeviceAttributeCanUse64BitStreamMemOpsV1                DeviceAttr = C.CU_DEVICE_ATTRIBUTE_CAN_USE_64_BIT_STREAM_MEM_OPS_V1              // Deprecated, along with v1 MemOps API, 64-bit operations are supported in ::cuStreamBatchMemOp and related APIs.
	...
	CuDeviceAttributeCanUseStreamMemOpsV1                     DeviceAttr = C.CU_DEVICE_ATTRIBUTE_CAN_USE_STREAM_MEM_OPS_V1                     // Deprecated, along with v1 MemOps API, ::cuStreamBatchMemOp and related APIs are supported.
	...
	CuDeviceAttributeCanUseStreamWaitValueNorV1               DeviceAttr = C.CU_DEVICE_ATTRIBUTE_CAN_USE_STREAM_WAIT_VALUE_NOR_V1              // Deprecated, along with v1 MemOps API, ::CU_STREAM_WAIT_VALUE_NOR is supported.
	...
	CuDeviceAttributeD3d12CigSupported                        DeviceAttr = C.CU_DEVICE_ATTRIBUTE_D3D12_CIG_SUPPORTED                           // Device supports CIG with D3D12.
	...
	CuDeviceAttributeGpuPciDeviceId                           DeviceAttr = C.CU_DEVICE_ATTRIBUTE_GPU_PCI_DEVICE_ID                             // The combined 16-bit PCI device ID and 16-bit PCI vendor ID.
	CuDeviceAttributeGpuPciSubsystemId                        DeviceAttr = C.CU_DEVICE_ATTRIBUTE_GPU_PCI_SUBSYSTEM_ID                          // The combined 16-bit PCI subsystem ID and 16-bit PCI subsystem vendor ID.
	CuDeviceAttributeHandleTypeFabricSupported                DeviceAttr = C.CU_DEVICE_ATTRIBUTE_HANDLE_TYPE_FABRIC_SUPPORTED                  // Device supports exporting memory to a fabric handle with cuMemExportToShareableHandle() or requested with cuMemCreate()
	...
	CuDeviceAttributeHostAllocDmaBufSupported                 DeviceAttr = C.CU_DEVICE_ATTRIBUTE_HOST_ALLOC_DMA_BUF_SUPPORTED                  // Device supports page-locked host memory buffer sharing with dma_buf mechanism.
	CuDeviceAttributeHostMemoryPoolsSupported                 DeviceAttr = C.CU_DEVICE_ATTRIBUTE_HOST_MEMORY_POOLS_SUPPORTED                   // Device suports HOST location with the ::cuMemAllocAsync and ::cuMemPool family of APIs
	CuDeviceAttributeHostNativeAtomicSupported                DeviceAttr = C.CU_DEVICE_ATTRIBUTE_HOST_NATIVE_ATOMIC_SUPPORTED                  // Link between the device and the host supports all native atomic operations
	CuDeviceAttributeHostNumaId                               DeviceAttr = C.CU_DEVICE_ATTRIBUTE_HOST_NUMA_ID                                  // NUMA ID of the host node closest to the device. Returns -1 when system does not support NUMA.
	CuDeviceAttributeHostNumaMemoryPoolsSupported             DeviceAttr = C.CU_DEVICE_ATTRIBUTE_HOST_NUMA_MEMORY_POOLS_SUPPORTED              // Device supports HOST_NUMA location with the ::cuMemAllocAsync and ::cuMemPool family of APIs
	CuDeviceAttributeHostNumaMultinodeIpcSupported            DeviceAttr = C.CU_DEVICE_ATTRIBUTE_HOST_NUMA_MULTINODE_IPC_SUPPORTED             // Device supports HOST_NUMA location IPC between nodes in a multi-node system.
	CuDeviceAttributeHostNumaVirtualMemoryManagementSupported DeviceAttr = C.CU_DEVICE_ATTRIBUTE_HOST_NUMA_VIRTUAL_MEMORY_MANAGEMENT_SUPPORTED // Device supports HOST_NUMA location with the virtual memory management APIs like ::cuMemCreate, ::cuMemMap and related APIs
	...
	CuDeviceAttributeHostVirtualMemoryManagementSupported     DeviceAttr = C.CU_DEVICE_ATTRIBUTE_HOST_VIRTUAL_MEMORY_MANAGEMENT_SUPPORTED      // Device supports HOST location with the virtual memory management APIs like ::cuMemCreate, ::cuMemMap and related APIs
	CuDeviceAttributeIntegrated                               DeviceAttr = C.CU_DEVICE_ATTRIBUTE_INTEGRATED                                    // Device is integrated with host memory
	CuDeviceAttributeIpcEventSupported                        DeviceAttr = C.CU_DEVICE_ATTRIBUTE_IPC_EVENT_SUPPORTED                           // Device supports IPC Events.
	...
	CuDeviceAttributeMemDecompressAlgorithmMask               DeviceAttr = C.CU_DEVICE_ATTRIBUTE_MEM_DECOMPRESS_ALGORITHM_MASK                 // The returned valued shall be interpreted as a bitmask, where the individual bits are described by the ::CUmemDecompressAlgorithm enum.
	CuDeviceAttributeMemDecompressMaximumLength               DeviceAttr = C.CU_DEVICE_ATTRIBUTE_MEM_DECOMPRESS_MAXIMUM_LENGTH                 // The returned valued is the maximum length in bytes of a single decompress operation that is allowed.
	CuDeviceAttributeMemSyncDomainCount                       DeviceAttr = C.CU_DEVICE_ATTRIBUTE_MEM_SYNC_DOMAIN_COUNT                         // Number of memory domains the device supports.
	CuDeviceAttributeMpsEnabled                               DeviceAttr = C.CU_DEVICE_ATTRIBUTE_MPS_ENABLED                                   // Indicates if contexts created on this device will be shared via MPS
	CuDeviceAttributeMulticastSupported                       DeviceAttr = C.CU_DEVICE_ATTRIBUTE_MULTICAST_SUPPORTED                           // Device supports switch multicast and reduction operations.
	...
	CuDeviceAttributeNumaConfig                               DeviceAttr = C.CU_DEVICE_ATTRIBUTE_NUMA_CONFIG                                   // NUMA configuration of a device: value is of type ::CUdeviceNumaConfig enum
	CuDeviceAttributeNumaId                                   DeviceAttr = C.CU_DEVICE_ATTRIBUTE_NUMA_ID                                       // NUMA node ID of the GPU memory
	CuDeviceAttributeOnlyPartialHostNativeAtomicSupported     DeviceAttr = C.CU_DEVICE_ATTRIBUTE_ONLY_PARTIAL_HOST_NATIVE_ATOMIC_SUPPORTED     // Link between the device and the host supports only some native atomic operations
	...
	CuDeviceAttributeTensorMapAccessSupported                 DeviceAttr = C.CU_DEVICE_ATTRIBUTE_TENSOR_MAP_ACCESS_SUPPORTED                   // Device supports accessing memory using Tensor Map.
	...
	CuDeviceAttributeUnifiedFunctionPointers                  DeviceAttr = C.CU_DEVICE_ATTRIBUTE_UNIFIED_FUNCTION_POINTERS                     // Device supports unified function pointers.
	...
	CuDeviceAttributeVulkanCigSupported                       DeviceAttr = C.CU_DEVICE_ATTRIBUTE_VULKAN_CIG_SUPPORTED                          // Device supports CIG with Vulkan.
