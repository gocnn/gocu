...
func DeviceComputeCapability(device Device) (major, minor int) {
	var maj, min C.int
	// Use cuDeviceGetAttribute instead of deprecated cuDeviceComputeCapability
	err := Result(C.cuDeviceGetAttribute(&maj, C.CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR, C.CUdevice(device)))
	if err != SUCCESS {
		panic(err)
	}
	err = Result(C.cuDeviceGetAttribute(&min, C.CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR, C.CUdevice(device)))
	if err != SUCCESS {
		panic(err)
	}
	major = int(maj)
...
func DeviceGetProperties(dev Device) (prop DevProp) {
	...
	// Use cuDeviceGetAttribute instead of deprecated cuDeviceGetProperties
	err := Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_BLOCK, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.MaxThreadsPerBlock = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_MAX_BLOCK_DIM_X, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.MaxThreadsDim[0] = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_MAX_BLOCK_DIM_Y, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.MaxThreadsDim[1] = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_MAX_BLOCK_DIM_Z, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.MaxThreadsDim[2] = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_X, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.MaxGridSize[0] = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_Y, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.MaxGridSize[1] = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_Z, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.MaxGridSize[2] = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_BLOCK, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.SharedMemPerBlock = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_TOTAL_CONSTANT_MEMORY, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.TotalConstantMemory = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_WARP_SIZE, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.SIMDWidth = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_MAX_PITCH, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.MemPitch = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_MAX_REGISTERS_PER_BLOCK, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.RegsPerBlock = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_CLOCK_RATE, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.ClockRate = int(value)

	err = Result(C.cuDeviceGetAttribute(&value, C.CU_DEVICE_ATTRIBUTE_TEXTURE_ALIGNMENT, C.CUdevice(dev)))
	if err != SUCCESS {
		panic(err)
	}
	prop.TextureAlign = int(value)
